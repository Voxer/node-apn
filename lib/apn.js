...
var Connection = function (optionArgs) {
	...
	var onDrain = function () {
		...
		while (writeBuffer.length && self.socket.socket.bufferSize == 0) {
			notification = writeBuffer.shift();
			writeNotificationToSocket(notification.data, notification.message_id);
		}
	};
	...
	var writeNotificationToSocket = function (data, message_id) {
		if (self.socket === undefined || self.socket.readyState != 'open') {
			logger.log("apn", "writing to socket but socket is not open message_id=" + message_id);
			if ((self.socket === undefined || self.socket.readyState == 'closed') && readyToConnect()) {
				logger.log("apn", "socket is not open but ready to connect attempted to restart socket message_id=" + message_id);
				startSocket();
			}
			bufferDataForWrite(data, message_id);
		} else {
			if (self.socket.socket.bufferSize > 0) {
				bufferDataForWrite(data, message_id);
			} else {
				self.socket.write(data, function () {
					logger.log("apn", "apns data written to socket for message_id=" + message_id);
				});
			}
		}
	};

	var bufferDataForWrite = function (data, message_id) {
		logger.log("apn", "buffering data while socket is closed for message_id=" + message_id);
		writeBuffer.push({data: data, message_id: message_id});
	};

	this.sendNotification = function (note, message_id) {
		var encoding = 'utf8';
		if (note.encoding) {
		...
		if (options.enhanced) {
			...
			cachedNotes.push({note: note, message_id: message_id});
			tidyCachedNotes();
		}
		else {
		...
		writeNotificationToSocket(data, message_id);
	};
	...
	var handleTransmissionError = function (data) {
		...
		if (data[0] == 8) {
			...
			for (var i = 0; i < count; i++) {
				note = cachedNotes.shift();
				self.sendNotification(note.note, note.message_id);
			}
		}
	}
};
