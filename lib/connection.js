 ...
 * @config {Number} [notificationLimit=Infinity] The maximum number of notifications each socket is allowed to have before they are destroyed.
 * @config {Number} [timeLimit=Infinity] The uptime (in milliseconds) that a socket will stay connected.
 */
...
Connection.prototype.createConnection = function() {
	...
	this.connect().delay(100).then(function () {
		...
		this.socket.apnConnectedAt = +new Date;
		if (this.options.timeLimit !== Infinity) {
			this.socket.apnDestroyTimeout =
					setTimeout(this.gracefullyDestroy.bind(this, this.socket),
...
Connection.prototype.errorOccurred = function(socket, err) {
	...
	this.gracefullyDestroy(socket);
};

/**
 * @private
 */
Connection.prototype.gracefullyDestroy = function gracefullyDestroy(socket) {
 ...
 * @private
 */
Connection.prototype.shouldDestroy = function shouldDestroy(socket) {
	// Limit number of notifications per socket
	if (socket.apnCurrentId > this.options.notificationLimit) {
		return true;
	}

	// Limit maximum socket uptime
	if (this.options.timeLimit !== false &&
			socket.apnConnectedAt + this.options.timeLimit <= +new Date) {
		return true;
	}
 ...
 */
 Connection.prototype.retireSocket = function(socket) {
	...
	if (socket.apnDestroyTimeout) {
		clearTimeout(socket.apnDestroyTimeout);
		socket.apnDestroyTimeout = null;
	}
